...
// TODO Proper typing for this section
...
const getRain = (data: WeatherApiResponse.Data) => {
...
const getSymbolCode = (data: WeatherApiResponse.Data) => {
...
const getMostOccurentWeatherSymbolInTheGivenData = (timeseries: WeatherApiResponse.Timesery[]) => {
    type A = [string, WeatherApiResponse.Timesery[]];
    let foundObject =
        Object.entries(timeseries.reduce<{ [key: string]: WeatherApiResponse.Timesery[] }>((acc, obj) => {
            let key = getSymbolCode(obj.data);
            // console.log(key)

            // get Symbol code for data
            if (!acc[key]) {
                acc[key] = []
            }
            acc[key].push(obj)
            ...
            .sort(([keyA, symbolsA]: A, [keyB, symbolsB]: A) => symbolsA.length - symbolsB.length)[0]
    return getSymbol(foundObject[0]);
}

const getPrettyPic = (symbolCode: keyof typeof icons) => icons[symbolCode];

const getAverage = (arr: WeatherApiResponse.Timesery[], func: (a: WeatherApiResponse.Timesery) => number) => {
    let reduction = getOccurrences(arr, func)
    return Math.round(reduction.sum / reduction.count)
}
const getOccurrences = (arr: WeatherApiResponse.Timesery[], func: (a: WeatherApiResponse.Timesery) => number) => {
    return arr
        .filter((data: WeatherApiResponse.Timesery) => new Date(data.time).getHours() >= 8 || new Date(data.time).getHours() <= 18)
        .reduce(({ sum, count }, b) => {
...
const getWindDay = (data: WeatherApiResponse.Timesery[]) => getAverage(data, a => a.data.instant.details.wind_speed);
const getWindDirDay = (data: WeatherApiResponse.Timesery[]) => {
...
const getRainDay = (data: WeatherApiResponse.Timesery[]) => getAverage(data, a => getRain(a.data))
...
export default function WeatherCard({ title, data: timeseries }: { title?: string, data: WeatherApiResponse.Timesery[] }) {
    const rain_icon = icons.rain;

    let date = new Date(timeseries[0].time);
    //todo refactor
    if (!title) title = DAYS[date.getDay()];


    const weatherSymbol = getMostOccurentWeatherSymbolInTheGivenData(timeseries);

    const filteredData4 = timeseries.filter(data => new Date(data.time).getHours() % 4 == 0)

    let timeseriesByHours = filteredData4.length < 6
        ? timeseries.length < 6
            ? timeseries
            : new Date(timeseries[0].time).getHours() % 2 == 0
                ? timeseries.filter(data => new Date(data.time).getHours() % 2 == 0).slice(0, 6)
                : [timeseries[0], ...timeseries.filter(data => new Date(data.time).getHours() % 2 == 0).slice(0, 5)]
        : filteredData4
    ...
    return <div className="weather-card">
        <div className="card-header">
            <div className="card-title-wrapper">
                ...
                <h1 className="card-temperature">
                    {getTemperatureDay(timeseries)}°C
                </h1>
            </div>
        ...
        <div className="card-subheader">
            <div className="wrapper-rain card-subheader-container">
                ...
                <p className="rain-text card-subheader-text">Srážky: <strong>{getTemperatureDay(timeseries)}&nbsp;mm</strong></p>
            </div>
            <div className="wrapper-wind card-subheader-container">
                <div className="card-symbol-small wind-icon">
                    <img src={wind_icon} alt="wind icon" />
                </div>
                <p className="wind-text card-subheader-text">Vítr rychlosti <strong>{getWindDay(timeseries)}</strong> m/s z{getWindDirDay(timeseries)}</p>
            </div>
        </div>
        <table className="card-forecast">
            ...
            <tbody>
                {timeseriesByHours.map((data, i) => <tr key={i}>
